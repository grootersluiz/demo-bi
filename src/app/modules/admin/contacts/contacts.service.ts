...
export class ContactsService {
     ...
     * Get user by Group
     */
    getUsersByDash(id: string): Observable<User[]> {
        return this._httpClient
            .get<User[]>(`http://10.2.1.108/v1/users?dashboardId=${id}`)
            .pipe(
                tap((users) => {
                    let orderedContacts = [...users['data']];
                    orderedContacts.sort((a, b) =>
     ...
     * Get users by report
     */
    getUsersByReport(id: string): Observable<User[]> {
        return this._httpClient
            .get<User[]>(`http://10.2.1.108/v1/users?reportId=${id}`)
            .pipe(
                tap((users) => {
                    let orderedContacts = [...users['data']];
                    orderedContacts.sort((a, b) =>
